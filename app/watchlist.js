...
//   DIVERGENCE (2026-07-04): the SELL path adds a queryGroupCov gate in
//   bestSellLead that logmon.py lacks — see QGROUP_COV_MIN. logmon.py still
//   mis-fires single-shared-word leads ("WTB Lodizal Shell Shield" -> my "Chitin
//   Shell Shield"); if that desktop monitor is ever used, port this gate over.
//
...
// [Tue Jun 09 03:14:02 2026] Soandso tells you, 'got that fungi?'  → a direct /tell TO you.
// Only "tells you" (incoming) — not group/guild/say and not "You told …" (outgoing).
const TELL_LINE_RE = /^\[(.*?)\]\s+(\S+)\s+tells you,\s+'(.*)'\s*$/;
function parseTellLine(raw) {
  const m = TELL_LINE_RE.exec(raw.replace(/[\r\n]+$/, ''));
  return m ? { ts: m[1], speaker: m[2], msg: m[3] } : null;
}
...
// DIVERGENCE FROM logmon.py (intentional, 2026-07-04): itemCov only asks "did the
// buyer's words cover MY item" — it fires when a rare shared word (Vermiculated,
// Sapphire, Purity) matches even though the buyer named a DIFFERENT item. This gate
// adds the reverse direction: of the single buyer item-group the match lands in,
// my item must explain most of its idf mass. Kills "Lodizal Shell Shield" ->
// my "Chitin Shell Shield" (only shell/shield shared; buyer's distinctive Lodizal
// is unexplained). Calibrated on real leads: false positives scored <=0.53,
// legit matches (incl. typo/abbrev) >=0.95, so 0.75 sits in a wide dead zone.
const QGROUP_COV_MIN = 0.75;
...
function expandAliases(seg, pats) {
  ...
  // Comma-join so each canonical name is its OWN phrase group (see queryGroupCov):
  // "cof, fungi" -> "... , Cloak of Flames , Fungus Covered Scale Tunic", not one
  // merged blob whose combined idf would dilute each item's coverage. score() is
  // comma-agnostic (its tokenizer drops commas) so this is inert for scoring.
  return extra.length ? seg + " , " + extra.join(" , ") : seg;
}

// DIVERGENCE gate for the SELL path (see QGROUP_COV_MIN). Fraction of the buyer's
// item-group idf mass that `candName` explains, maxed over groups — i.e. "of the
// one item the buyer named here, how much is really my item". `groups` is a
// precomputed phraseTokenGroups(seg). A high-idf buyer word my item can't match
// (their "Lodizal") drops the ratio; zero-idf noise (pst, x2, prices) is inert.
function queryGroupCov(groups, candName, idf) {
  const cToks = tokenize(candName);
  if (!cToks.length || !idf) return 0.0;
  let best = 0.0;
  for (const g of groups) {
    let groupIdf = 0.0, earned = 0.0, matched = false;
    for (const q of new Set(g)) {
      const w = idf.get(q) || 0.0;
      groupIdf += w;
      let sim = 0.0;
      for (const c of cToks) { const s = tokenSim(q, c); if (s > sim) sim = s; }
      if (sim > 0) matched = true;
      earned += sim * w;
    }
    if (matched && groupIdf > 0) best = Math.max(best, earned / groupIdf);
  }
  return best;
}
...
function bestSellLead(seg, candidates, idf, aliasPats) {
  ...
  const groups = phraseTokenGroups(seg);   // buyer item-groups, for the two-sided gate
  ...
  for (const name of candidates) {
    ...
    if (!conf) continue;
    // Reject a match where the buyer actually named a DIFFERENT item that merely
    // shares a word with mine (see QGROUP_COV_MIN).
    if (queryGroupCov(groups, name, idf) < QGROUP_COV_MIN) continue;
    if (best === null || s.total > best.total) best = { total: s.total, tier: conf, item: name };
  }
...
const WL = {
  ...
  segments, sellSegments, buySegments, parseAuctionLine, parseTellLine, watchlistHits,
  buildIdf, seqRatio, tokenSim, score, confidence,
  DEFAULT_ALIASES, compileAliases, expandAliases, queryGroupCov, bestSellLead, matchLine,
