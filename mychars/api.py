...
def load_compositions(conn, chars):
    ...
    for comp in comps:
        rows = {r["slot_index"]: r["character_id"] for r in conn.execute(
            "SELECT slot_index, character_id FROM composition_slots WHERE composition_id=?",
            (comp["id"],)) if 0 <= r["slot_index"] < 24}
...
def push_loginset(conn, char_ids):
    """Write the selected one-per-account login set to MQ config as a Lua data file."""
    if not char_ids:
        return 400, {"ok": False, "error": "Login set is empty."}
    marks = ",".join("?" * len(char_ids))
    rows = dbm.rows(conn, """SELECT c.name, c.class_name, c.level, a.alias, a.autologin_group,
                                    a.launch_order
    ...
    return 200, {"ok": True, "path": path, "members": [r["name"] for r in rows]}
...
def save_composition(conn, payload, comp_id=None):
    ...
    for i, cid in enumerate(slots):
        conn.execute("INSERT INTO composition_slots(composition_id, slot_index, character_id)"
                     " VALUES (?,?,?)", (comp_id, i, cid))
    conn.commit()
